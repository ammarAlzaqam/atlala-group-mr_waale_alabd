        ...
        <div className="relative z-2 bg-white rounded-xl shadow-lg shadow-primary-500/20 py-6 flex flex-wrap flex-col sm:flex-row items-center sm:items-stretch gap-y-6 overflow-hidden">
          ...
          {advList.map(({ title, icon }, index) => (
            <div
              key={title}
              className={clsx(
                "relative z-2 lg:flex-1 border-b sm:border-b-0 border-l-0 sm:border-l border-black/5 flex flex-col items-center gap-1 w-[70%] sm:w-[50%] md:w-[25%] lg:w-auto pb-5 sm:pb-0",
            ...
            >
              <img
                src={icon}
                alt="icon"
                className="w-10 sm:w-8 z-2 blue-img-filter"
              />
              <p className="z-2 text-center font-semibold sm:text-sm text-primary-700! sm:max-w-[80%]">
                {title}
              </p>
              <div className="absolute top-0 left-0 inset-0 bg-[url('/images/favicon.png')] bg-contain bg-center bg-no-repeat blue-img-filter opacity-15 z-1"></div>
            </div>
          ))}
          <div className="absolute top-0 left-0 inset-0 bg-[url('/images/stars.png')] blue-img-filter opacity-20 z-1"></div>
        </div>
